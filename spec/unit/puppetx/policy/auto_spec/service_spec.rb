...
require 'puppetx/policy/auto_spec/service'

autospec = Puppetx::Policy::AutoSpec

describe 'the Service auto_spec plugin' do
  it 'should be loaded' do
    autospec.auto_specs.size.should_not == 0
    autospec.auto_specs.select { |a| a[:type] == 'Service' }.size.should == 1
  end

  context 'when using the plugin' do
    it 'should write a spec file' do
      File.stubs(:open) # Catch other calls
      File.expects(:open).with('/foo/bar/service_spec.rb', 'w').returns true
