...
require 'puppetx/policy/auto_spec/group'

autospec = Puppetx::Policy::AutoSpec

describe 'the Group auto_spec plugin' do
  it 'should be loaded' do
    autospec.auto_specs.size.should_not == 0
    autospec.auto_specs.select { |a| a[:type] == 'Group' }.size.should == 1
  end

  context 'when using the plugin' do
    it 'should write a spec file' do
      File.stubs(:open) # Catch other calls
      File.expects(:open).with('/foo/bar/group_spec.rb', 'w').returns true
